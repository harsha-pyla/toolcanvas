/**
 * Social Media Card — Archetype E
 * Debounced live preview, flat QR, no gradients/shadows
 */
document.addEventListener('DOMContentLoaded', () => {
    ...
    function parseUrlParams() {
        ...
        state = {
            ...
            customColor: params.get('col') || '#3F6B4F',
            typography: params.get('ty') || 'sans',
            orientation: params.get('or') || 'horizontal',
            links: []
        };
        const linksStr = params.get('l');
        if (linksStr) { try { state.links = JSON.parse(linksStr); } catch(e){} }
    }

    function loadState() {
        try {
            const saved = localStorage.getItem('toolcanvas_sm_card_state');
            if (saved) { state = JSON.parse(saved); state = Object.assign({}, DEFAULT_DATA, state); }
            else { state = JSON.parse(JSON.stringify(DEFAULT_DATA)); }
        } catch(e){ state = JSON.parse(JSON.stringify(DEFAULT_DATA)); }
    }

    function saveState() {
        if (isPublicView) return;
        try { localStorage.setItem('toolcanvas_sm_card_state', JSON.stringify(state)); } catch(e){}
    }
    ...
    function syncStateToInputs() {
        ...
        const typo = document.querySelector(`input[name="card-typography"][value="${state.typography}"]`);
        if (typo) typo.checked = true;
        const orient = document.querySelector(`input[name="card-orientation"][value="${state.orientation}"]`);
    ...
    function toggleCustomColorVisibility() {
        ...
        if (wrapper) wrapper.style.display = state.theme === 'custom' ? 'flex' : 'none';
    }
    ...
    function getShareUrl() {
        ...
        if (state.theme === 'custom') params.set('col', state.customColor);
        params.set('ty', state.typography);
        params.set('or', state.orientation);
        if (state.links && state.links.length > 0) params.set('l', JSON.stringify(state.links));
    ...
    function updateQrCode() {
        ...
        // ensure flat ink after library may set gradient - force flat by setting fill style
        const canvas=document.getElementById('preview-qr-canvas');
        if(canvas) canvas.style.border='1px solid #DCDAD2';
    }
    ...
    function renderCardPreview() {
        ...
        if (separatorEl) separatorEl.style.display = (state.title && state.company) ? 'inline' : 'none';

        const emailEl = document.getElementById('preview-email');
        const emailRow = document.getElementById('contact-email-row');
        if (emailEl && emailRow) { if (state.email) { emailEl.textContent = state.email; emailRow.style.display = 'flex'; } else emailRow.style.display = 'none'; }
        const phoneEl = document.getElementById('preview-phone');
        const phoneRow = document.getElementById('contact-phone-row');
        if (phoneEl && phoneRow) { if (state.phone) { phoneEl.textContent = state.phone; phoneRow.style.display = 'flex'; } else phoneRow.style.display = 'none'; }
        const websiteEl = document.getElementById('preview-website');
        const websiteRow = document.getElementById('contact-website-row');
        if (websiteEl && websiteRow) { if (state.website) { websiteEl.textContent = state.website; websiteRow.style.display = 'flex'; } else websiteRow.style.display = 'none'; }
        const locationEl = document.getElementById('preview-location');
        const locationRow = document.getElementById('contact-location-row');
        if (locationEl && locationRow) { if (state.location) { locationEl.textContent = state.location; locationRow.style.display = 'flex'; } else locationRow.style.display = 'none'; }

        const contactsSection = document.getElementById('card-contacts-section');
        if (contactsSection) contactsSection.style.display = (state.email || state.phone || state.website || state.location) ? 'flex' : 'none';
        ...
        if (imgEl && placeholderEl) {
            if (state.avatar) { imgEl.src = state.avatar; imgEl.style.display = 'block'; placeholderEl.style.display = 'none'; if (btnClearPhoto) btnClearPhoto.style.display = 'inline-block'; }
            else { imgEl.src = ''; imgEl.style.display = 'none'; placeholderEl.style.display = 'flex'; if (btnClearPhoto) btnClearPhoto.style.display = 'none'; }
        }
        mockup.className = `social-card-box theme-${state.theme} layout-${state.orientation} font-${state.typography}`;
        if (state.theme === 'custom') mockup.style.setProperty('--accent-color', state.customColor);
        else mockup.style.removeProperty('--accent-color');
        ...
        if (socialsBar) {
            ...
            if (state.links && state.links.length > 0) {
                state.links.forEach(link => {
                    ...
                    a.href = link.url; a.target = '_blank'; a.rel = 'noopener noreferrer'; a.title = capitalize(link.platform);
                    a.innerHTML = SVG_ICONS[link.platform] || '🔗';
                    socialsBar.appendChild(a);
                });
                socialsBar.style.display = 'flex';
            } else socialsBar.style.display = 'none';
        }
        adjustCardScale();
    }
    ...
    function renderEditorLinksList() {
        ...
        const maxReached = state.links.length >= 5;
        if (addBtn) addBtn.disabled = maxReached;
        if (linkUrlInput) linkUrlInput.disabled = maxReached;
        ...
        state.links.forEach((link, index) => {
            ...
            item.innerHTML = `<div class="link-entry-details"><span class="platform-icon-indicator">${getEmojiIndicator(link.platform)}</span><strong>${capitalize(link.platform)}:</strong><span class="link-url-text">${link.url}</span></div><button type="button" class="btn-remove-link" data-index="${index}">Remove</button>`;
            item.querySelector('.btn-remove-link').addEventListener('click', () => {
                state.links.splice(index, 1); saveState(); renderCardPreview(); renderEditorLinksList(); updateQrCode();
            });
    ...
    function addLink() {
        ...
        if (state.links.length >= 5) { alert('Maximum 5 social links allowed.'); return; }
        state.links.push({ platform, url }); inputLinkUrl.value = '';
        saveState(); renderCardPreview(); renderEditorLinksList(); updateQrCode();
    }
    ...
    function bindInputs() {
        ...
        const debouncedRender = debounce(()=>{
            saveState(); renderCardPreview(); updateQrCode();
            // sync alias hidden inputs
            const aliasName=document.getElementById('name'); if(aliasName) aliasName.value=state.name;
            const aliasTitle=document.getElementById('title'); if(aliasTitle) aliasTitle.value=state.title;
        },80);
        ...
        // alias inputs also update main
        const aliasNameEl=document.getElementById('name');
        if(aliasNameEl && aliasNameEl!==nameInput) aliasNameEl.addEventListener('input', e=>{ state.name=e.target.value.trim(); if(nameInput) nameInput.value=state.name; debouncedRender(); });
        const aliasTitleEl=document.getElementById('title');
        if(aliasTitleEl && aliasTitleEl!==titleInput) aliasTitleEl.addEventListener('input', e=>{ state.title=e.target.value.trim(); if(titleInput) titleInput.value=state.title; debouncedRender(); });

        if (customColorInput) {
            customColorInput.addEventListener('input', debounce((e) => {
                state.customColor = e.target.value; saveState(); renderCardPreview();
            },80));
        }
        ...
        function handleAvatarFile(file){
            ...
            const localImg = avatarImg; const localPlaceholder = avatarPlaceholder; const localClearBtn = btnClearPhoto;
            reader.onload = (event) => {
                const base64Data = event.target.result;
                state.avatar = base64Data; saveState();
                if (localImg) { localImg.src = base64Data; localImg.style.display = 'block'; }
                if (localPlaceholder) localPlaceholder.style.display = 'none';
                if (localClearBtn) localClearBtn.style.display = 'inline-block';
                renderCardPreview();
            };
        ...
        if (avatarAlias) {
            avatarAlias.addEventListener('change', (e) => {
                const file = e.target.files[0];
                if (file) handleAvatarFile(file);
                avatarAlias.value = '';
            });
        ...
        if (dropzone) {
            dropzone.addEventListener('click', ()=> { if(avatarInput) avatarInput.click(); });
            dropzone.addEventListener('keydown', e=>{ if(e.key==='Enter'||e.key===' '){ e.preventDefault(); if(avatarInput) avatarInput.click(); }});
        ...
        if (btnClearPhoto) {
            btnClearPhoto.addEventListener('click', () => {
                state.avatar = ''; saveState();
                if (avatarImg) { avatarImg.src = ''; avatarImg.style.display = 'none'; }
                if (avatarPlaceholder) avatarPlaceholder.style.display = 'flex';
                btnClearPhoto.style.display = 'none'; renderCardPreview();
            });
        }

        document.querySelectorAll('input[name="card-theme"]').forEach(radio => {
            radio.addEventListener('change', (e) => {
                state.theme = e.target.value; saveState(); toggleCustomColorVisibility(); renderCardPreview(); updateQrCode();
            });
        });
        document.querySelectorAll('input[name="card-typography"]').forEach(radio => {
            radio.addEventListener('change', (e) => { state.typography = e.target.value; saveState(); renderCardPreview(); updateQrCode(); });
        });
        document.querySelectorAll('input[name="card-orientation"]').forEach(radio => {
            radio.addEventListener('change', (e) => { state.orientation = e.target.value; saveState(); renderCardPreview(); updateQrCode(); });
        });

        const addBtn = document.getElementById('btn-add-link');
        if (addBtn) addBtn.addEventListener('click', addLink);
        const inputLinkUrl = document.getElementById('input-link-url');
        if (inputLinkUrl) inputLinkUrl.addEventListener('keypress', (e) => { if (e.key === 'Enter') { e.preventDefault(); addLink(); } });
        ...
        if (btnReset) {
            btnReset.addEventListener('click', () => {
                if (confirm('Are you sure you want to reset all visiting card settings to default?')) {
                    ...
                    saveState(); syncStateToInputs(); toggleCustomColorVisibility(); renderCardPreview(); renderEditorLinksList(); updateQrCode();
                }
            });
        }
        ...
        if (btnDownload) {
            btnDownload.addEventListener('click', () => {
                ...
                btnDownload.innerHTML = '<span>Rendering PNG...</span>'; btnDownload.disabled = true;
                setTimeout(() => {
                    html2canvas(mockup, { useCORS: true, allowTaint: true, scale: 2, backgroundColor: null }).then(canvas => {
                        ...
                        console.error(err); alert('Could not export image.'); btnDownload.innerHTML = originalText; btnDownload.disabled = false;
                    });
                }, 100);
            });
        }
    ...
    function bindExtras(){
        ...
        if(lightBtn && darkBtn && frame){
            lightBtn.addEventListener('click', ()=>{
                lightBtn.classList.add('active'); darkBtn.classList.remove('active'); frame.classList.remove('dark');
            });
            darkBtn.addEventListener('click', ()=>{
                darkBtn.classList.add('active'); lightBtn.classList.remove('active'); frame.classList.add('dark');
            });
        }
        // QR canvas alias preservation: ensure element id preview-qr-canvas exists already
        // alias for theme toggles id avatars
    }
    ...
    function adjustCardScale() {
        const mockup = document.getElementById('social-card-mockup');
        const wrapper = mockup ? mockup.parentElement : null;
        if (!mockup || !wrapper) return;
        // no scale flourish — keep instant
