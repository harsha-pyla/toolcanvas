/* =========================================
   ToolCanvas — JPG to PNG script.js
   ========================================= */
...
(function () {
    ...
    function processFiles(files) {
        ...
        if (validCount === 0) {
            if (window.showToast) window.showToast('Please select JPG/JPEG files only');
            return;
        }
        ...
        if (filesQueue.length === validCount + startQueueLen - validCount) {
            // first load
            enableResize.checked = false;
            resizeFields.style.display = 'none';
            downloadAllBtn.style.display = 'none';
        }
        // Simpler: always reset if first time
        if (startQueueLen === 0) {
            enableResize.checked = false;
            resizeFields.style.display = 'none';
            downloadAllBtn.style.display = 'none';
    ...
    function createFileCardDOM(obj) {
        ...
        if (obj.status === 'ready') {
            ...
            downloadBtn.download = `${baseName}_converted.png`;
    ...
    function convertAllImages() {
        ...
        filesQueue.forEach(function (obj) {
            ...
            setTimeout(function() {
                ...
                // Clear transparent canvas for PNG
                ...
                obj.convertedDataUrl = pngDataUrl;
                obj.status = 'done';
